...
selected_columns <- c("h1", "h2", "h3", "d1", "d2", "d3", "d4", "d5")
...
esm_cortisol_base <- esm_cortisol_base %>%
  mutate(d_meas = as.numeric(d1) + as.numeric(d2) + as.numeric(d3) + as.numeric(d4) + as.numeric(d5)) %>% # + as.numeric(d5)
  group_by(ID) %>%
...
##################################
#MEDIATION_DATA
update_mediation_data <- read_sav("data/mediation_data.sav")
mediation_data <- data.frame(ID = update_mediation_data$ID)

mediation_data <- mediation_data %>%
  left_join(all_base, by = "ID")


mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, esm_PLEs_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, Rumination_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1a_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1b_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1c_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1d_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1e_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1f_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress2_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, anxious_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, down_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, lonely_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, insecure_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, annoyed_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, as_sum_week, ta_sum_week, h_sum_week, d_sum_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, esm_PLEs_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1a_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1b_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1c_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1d_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1e_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1f_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress1_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, socstress2_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, anxious_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, down_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, lonely_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, insecure_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, annoyed_mean_week)), by = "ID")
mediation_data <- left_join(mediation_data, distinct(select(esm_cortisol_base, ID, as_mean_week, ta_mean_week, h_mean_week, d_mean_week)), by = "ID")
...
###################
#UPDATE MEDIATION DATA
update_mediation_data <- read_sav("data/data_main/to_update_mediation_data.sav")


mediation_data <- mediation_data %>%
  rename(
    ESM_PLEs_sum_week = esm_PLEs_sum_week,
    PLE_me = esm_PLEs_mean_week
  )


tmp_update_mediation_data <- update_mediation_data %>%
  left_join(
    mediation_data %>%
      select(ID, ESM_PLEs_sum_week, PLE_me, as_sum_week, ta_sum_week, h_sum_week, d_sum_week,
             as_mean_week, ta_mean_week, h_mean_week, d_mean_week),
    by = "ID"
  ) %>%
  mutate(
    ESM_PLEs_sum_week = coalesce(ESM_PLEs_sum_week.y, ESM_PLEs_sum_week.x),
    PLE_me = coalesce(PLE_me.y, PLE_me.x),
    as_sum_week = coalesce(as_sum_week.y, as_sum_week.x),
    ta_sum_week = coalesce(ta_sum_week.y, ta_sum_week.x),
    h_sum_week = coalesce(h_sum_week.y, h_sum_week.x),
    d_sum_week = coalesce(d_sum_week.y, d_sum_week.x),
    as_mean_week = coalesce(as_mean_week.y, as_mean_week.x),
    ta_mean_week = coalesce(ta_mean_week.y, ta_mean_week.x),
    h_mean_week = coalesce(h_mean_week.y, h_mean_week.x),
    d_mean_week = coalesce(d_mean_week.y, d_mean_week.x)
  ) %>%
...
current_time <- format(Sys.time(), "%Y%m%d_%H%M%S")
# Tworzenie nazwy pliku z aktualną datą i czasem
filename <- paste0("out/isd5_mediation_data_", current_time, ".sav")
# Zapis danych
write_sav(tmp_update_mediation_data, filename)
...
alpha_crombach <- esm_cortisol_base %>%
...
current_time <- format(Sys.time(), "%Y%m%d_%H%M%S")
# Tworzenie nazwy pliku z aktualną datą i czasem
filename <- paste0("out/alpha_crombach_", current_time, ".sav")
# Zapis danych
write_sav(alpha_crombach, filename)